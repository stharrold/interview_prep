...
def test_get_next_path(current_path=[1, 2, 2], next_path=[2, 1, 1]):
    ...
    """
    assert _get_next_path(current_path=current_path) == next_path
    return None
...
def test_get_next_node_path_values(
    ...
    """
    for (test, ref) in zip(get_next_node_path_values(bin_tree=bin_tree, current_path=current_path),
                           (next_node, next_path, next_values)):
        assert test == ref
    return None


def test_is_valid_bin_search_tree(bin_tree=[50,
                                           [30, [10, None, None], [40, None, None]],
                                           [70, [60, None, None], [80, None, None]]],
                             is_bst=True):
    """pytest style test for is_valid_bin_search_tree.

    """
    assert is_valid_bin_search_tree(bin_tree=bin_tree) == is_bst
    return None


test_is_valid_bin_search_tree(bin_tree=[50,
                                       [30, [10, None, None], [60, None, None]],
                                       [70, [60, None, None], [80, None, None]]])
